...
# Updated for PostgreSQL table names
EXPORT_TO_FIELDS = {
    'circ_elem_export.sql': {
        'table_name': 'inventory_circuit_element',
        'fields': [
    ...
    'copper_export.sql': {
        'table_name': 'inventory_copper_service',
        'fields': [
    ...
    'cross_conn_export.sql': {
        'table_name': 'inventory_cross_connection',
        'fields': [
    ...
    'equipment_export.sql': {
        'table_name': 'inventory_equipment',
        'fields': [
    ...
    'fttx_export.sql': {
        'table_name': 'inventory_fttx_service',
        'fields': [
...
def convert_sql_file(file_path, table_config):
    """Convert Oracle export SQL to PostgreSQL format"""
    ...
    field_list = table_config['fields']
    col_map = build_column_map(field_list)
    
    # Remove Oracle-specific lines
    lines = content.split('\n')
    new_lines = []
    
    for line in lines:
        # Skip Oracle-specific comments and commands
        ...
        if 'Insert into' in line or 'INSERT INTO' in line:
            # Replace EXPORT_TABLE with actual table name
            line = re.sub(r'Insert\s+into\s+EXPORT_TABLE', f'INSERT INTO {table_name}', line, flags=re.IGNORECASE)
            ...
            line = re.sub(r'(INSERT\s+INTO\s+[^(]+)\(([^)]+)\)', replace_columns, line, flags=re.IGNORECASE)
...
def main():
    ...
    for filename, table_config in EXPORT_TO_FIELDS.items():
        sql_file = sql_dir / filename
        
        if sql_file.exists():
            try:
                convert_sql_file(sql_file, table_config)
                converted_count += 1
    ...
    print(f"\n🎉 Converted {converted_count}/{len(EXPORT_TO_FIELDS)} files")
